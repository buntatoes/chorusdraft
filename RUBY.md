...
**0.51.3 preview** · Linux, macOS, and Windows · Ruby 4.0+
...
From a combined download, open the root launcher and choose **Ruby**, then
**Bluesky** or **Mastodon**, then **Set up configuration**.
From a standalone Ruby package, run `./bot setup` on Linux/macOS or
`.\bot.bat setup` on Windows. Edit the generated `.env` before drafting.

The Ruby configuration is in `bluesky/.env` or `mastodon/.env` in a combined
download, and beside the launcher in a standalone package. Setup preserves
...
the new version into a new directory, and securely copy your `.env`, configured target and
