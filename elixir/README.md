...
0.51.1 command workflows, optional Jetstream, state import, setup, and one
Linux package containing both Bluesky and Mastodon modes. [PARITY.md](PARITY.md)
maps these features to the read-only Ruby 0.51.1 baseline and their verification. The [Elixir verification workflow](https://github.com/buntatoes/chorusdraft/actions/workflows/elixir.yml)
passed 64 tests using fixtures and loopback servers in
[the verified Linux run](https://github.com/buntatoes/chorusdraft/actions/runs/34012077368). CI also imported state
generated by the pinned Ruby reference, installed the combined package, checked
both platform modes, and rebuilt the shipped sources offline. It used Ubuntu
22.04, OTP 25.3, and Elixir 1.15.8 without live account credentials or publication.
