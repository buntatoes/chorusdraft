defmodule ChorusDraft.Guard.PII do
  @moduledoc """
  Proprietary ChorusDraft Guard personal-information screening.
  Not licensed under Apache 2.0. See `elixir/guard/LICENSE`.
  Conservative pattern screening, not a guarantee that text is anonymous.
  """
  alias ChorusDraft.Error
  alias ChorusDraft.Guard.Safety
  ...
  # Whitespace is consumed by exactly one branch per separator so runs of spaces
  # match in linear time.
  @obfuscated_email ~r/(?<![\p{L}\p{N}_@])[\p{L}\p{N}._%+-]+(?:\s*(?:\[at\]|\(at\)|\{at\})\s*|\s+(?:at|@)\s+)[\p{L}\p{N}-]+(?:(?:\s*(?:\[dot\]|\(dot\)|\{dot\}|\.)\s*|\s+dot\s+)[\p{L}\p{N}-]+)+/iu
  ...
  @credential ~r/\b(?:AIza[\w-]{35}|gh[pousr]_[\w]{20,}|github_pat_[\w]{20,}|sk-(?:proj-)?[\w-]{20,}|[sr]k_(?:live|test)_\w{16,}|AKIA[0-9A-Z]{16}|eyJ[\w-]{10,}\.[\w-]{10,}\.[\w-]{10,})\b/u
  # Bluesky app passwords: four lowercase groups of four.
  @app_password ~r/\b[a-z0-9]{4}(?:-[a-z0-9]{4}){3}\b/u
  @private_key ~r/-----BEGIN [A-Z ]*PRIVATE KEY-----/u
  @patterns [
    @email,
    @obfuscated_email,
    @number,
